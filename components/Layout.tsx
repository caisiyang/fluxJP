...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F7F6F2] text-slate-800 font-sans flex flex-col relative overflow-hidden">
      {/* Main Content */}
      ...
      {!sessionType && (
        <nav className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto bg-transparent z-50 pointer-events-none">
          ...
          <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[#F7F6F2] via-[#F7F6F2]/95 to-transparent pointer-events-none" />
                ...
                {({ isActive }) => (
                  <>
                    <div className={clsx(
                      ...
                      isActive ? "text-[#D45D5D]" : "text-slate-300"
                    )}>
                      {isActive && (
                    ...
                    <span className={clsx(
                      ...
                      isActive ? "text-[#D45D5D] opacity-100" : "text-slate-300 opacity-80"
                    )}>
                      {item.name}
                    </span>
                  </>
