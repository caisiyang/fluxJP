...
export const MemoryPipeline: React.FC = () => {
    const stats = useLiveQuery(async (): Promise<PipelineStats> => {
        ...
        const newCount = words.filter(w => w.status === WordStatus.NEW).length;
        const learningCount = words.filter(w => w.status === WordStatus.LEARNING).length;
        const leechCount = words.filter(w => w.status === WordStatus.LEECH).length;
        const reviewWords = words.filter(w => w.status === WordStatus.REVIEW);
    ...
    const stages = [
        { key: 'new', label: '未学習', count: stats.new, color: 'bg-slate-400', textColor: 'text-slate-500 dark:text-[#888]' },
        { key: 'learning', label: '学習中', count: stats.learning, color: 'bg-amber-400', textColor: 'text-amber-600 dark:text-amber-400' },
        { key: 'shortTerm', label: '短期記憶', count: stats.shortTerm, color: 'bg-blue-400', textColor: 'text-blue-600 dark:text-blue-400' },
        { key: 'longTerm', label: '長期記憶', count: stats.longTerm, color: 'bg-indigo-500', textColor: 'text-indigo-600 dark:text-indigo-400' },
        { key: 'mastered', label: '習得済', count: stats.mastered, color: 'bg-emerald-500', textColor: 'text-emerald-600 dark:text-emerald-400' },
    ];

    return (
        <div className="bg-[#F7F6F2] dark:bg-[#2a2a2a] rounded-2xl border border-[#E8E6E0] dark:border-[#3a3a3a] p-5 shadow-sm">
            <h2 className="text-sm font-bold text-slate-700 dark:text-[#e5e5e0] mb-4">記憶の定着度</h2>
            ...
            <div className="mt-4 pt-4 border-t border-[#E8E6E0] dark:border-[#3a3a3a] text-center">
                <span className="text-xs text-slate-400 dark:text-[#888]">総単語数：</span>
